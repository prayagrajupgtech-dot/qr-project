    ...
    <>
      <div>
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center font-black text-black shadow-lg shadow-amber-500/20 text-lg shrink-0">
            AD
          </div>
          <div className="min-w-0">
            <h1 className="font-black text-sm uppercase tracking-wider text-white truncate">Admin Panel</h1>
            <p className="text-[10px] text-amber-400 font-bold uppercase tracking-widest truncate">Maurya System</p>
          </div>
        </div>
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-[60] w-10 h-10 rounded-xl bg-amber-500 text-black flex items-center justify-center font-black text-lg shadow-lg"
      >
        {mobileOpen ? "✕" : "☰"}
      </button>
      ...
      {/* Desktop sidebar */}
      <aside className={`hidden lg:flex w-64 border-r flex-col justify-between shrink-0 min-h-screen p-6 ${isDark ? "bg-slate-950 border-white/10" : "bg-[#052e16] border-[#065f46]"}`}>
        {sidebarContent}
      </aside>

      {/* Mobile sidebar */}
      <aside className={`lg:hidden fixed inset-y-0 left-0 z-50 w-72 flex flex-col p-6 transition-transform duration-300 ${
        mobileOpen ? "translate-x-0" : "-translate-x-full"
      } ${isDark ? "bg-slate-950 border-r border-white/10" : "bg-[#052e16] border-r border-[#065f46]"}`}>
